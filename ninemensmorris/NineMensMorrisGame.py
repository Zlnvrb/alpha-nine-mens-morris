...
from .NineMensMorrisLogic import Board, stones_ix, moves_ix
import sys
import numpy as np
...
class NineMensMorrisGame(Game):
    ...
    def __init__(self):
        ...
        self.MAX_MOVES_WITHOUT_MILL = 40
    ...
    def getGameEnded(self, board, player) -> float:
        ...
        if b.get_moves_made_without_mill() >= self.MAX_MOVES_WITHOUT_MILL:
            # print(f"DRAW! Moves made: {b.get_moves_made_without_mill()}")
            return -0.001
    ...
    def getCanonicalForm(self, board, player):
        ...
        canon_board[stones_ix] = count_placements
        canon_board[moves_ix] = current_moves
    ...
    def stringRepresentation(self, board):
        ...
        b = Board(board)
        board_array = b.get_board_as_array()

        board_s = ""
        for v in board_array:
            board_s += str(v) + ","

        stones = b.get_stones_placed()
        moves = b.get_moves_made_without_mill()

        board_s += "," + str(stones) + "," + str(moves)
