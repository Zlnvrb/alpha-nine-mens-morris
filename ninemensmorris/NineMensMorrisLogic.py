...
class Board():
    ...
    def map_board_array_to_image(self, board):
        image = np.zeros((self.n, self.m), dtype=int)
        for i in range(0, len(board)):
            v = board[i]
            p = placement_mapping[i]
            image[p] = v
        return image
    ...
    def get_board_as_array(self):
        ...
        """
        board_array = np.zeros(24, dtype=int)
        for pos in placement_mapping.keys():
            image_pos = placement_mapping[pos]
            v = self.pieces[image_pos]
            board_array[pos] = v
    ...
    def get_stones_placed(self):
        ...
        """
        # print(f"STONES {self.pieces[stones_ix]}")
        return self.pieces[stones_ix]

    def get_moves_made_without_mill(self):
        """
        :return: number of moves made thus far without a mill being created.
        """
        # print(f"MOVES {self.pieces[moves_ix]}")
        return self.pieces[moves_ix]
    ...
    def to_board(self, board_array, misc):
        ...
        image = self.map_board_array_to_image(board_array)
        # print(f'Image {image}')
        image[stones_ix] = misc[0]
        image[moves_ix] = misc[1]
